...
def ensure_confirmed(nodes, populate_backlog=False):
    @retry(delay=2)
...
class NanoNet:
    def __init__(self):
        self.runid = str(datetime.now()).replace(" ", "_")
    ...
    def create_node(
        ...
        cpuset_cpus = str(CPUS_PER_NODE) if limit_cpus else None
